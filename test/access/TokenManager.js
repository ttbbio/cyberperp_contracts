...
const { deployContract, deployTimelock, deployCybTimelock } = require("../shared/fixtures")
...
describe("TokenManager", function () {
  ...
  beforeEach(async () => {
    ...
    cybTimelock = await deployCybTimelock([
      wallet.address,
      5 * 24 * 60 * 60,
      7 * 24 * 60 * 60,
      user0.address,
  ...
  it("approve", async () => {
    ...
    await expect(tokenManager.connect(wallet).approve(cyb.address, user2.address, expandDecimals(5, 18), 1))
      .to.be.revertedWith("TokenManager: action not signalled")
  ...
  it("signalSetGov", async () => {
    await expect(tokenManager.connect(user0).signalSetGov(timelock.address, cyb.address, user1.address))
      .to.be.revertedWith("TokenManager: forbidden")

    await tokenManager.connect(wallet).signalSetGov(timelock.address, cyb.address, user1.address)
  })

  it("signSetGov", async () => {
    await expect(tokenManager.connect(user0).signSetGov(timelock.address, cyb.address, user1.address, 1))
      .to.be.revertedWith("TokenManager: forbidden")

    await expect(tokenManager.connect(signer2).signSetGov(timelock.address, cyb.address, user1.address, 1))
      .to.be.revertedWith("TokenManager: action not signalled")
    ...
    await tokenManager.connect(signer2).signSetGov(timelock.address, cyb.address, user1.address, 1)

    await expect(tokenManager.connect(signer2).signSetGov(timelock.address, cyb.address, user1.address, 1))
      .to.be.revertedWith("TokenManager: already signed")

    await tokenManager.connect(signer1).signSetGov(timelock.address, cyb.address, user1.address, 1)
  })

  it("setGov", async () => {
    await cyb.setGov(cybTimelock.address)

    await expect(tokenManager.connect(user0).setGov(cybTimelock.address, cyb.address, user1.address, 1))
      .to.be.revertedWith("TokenManager: forbidden")

    await expect(tokenManager.connect(wallet).setGov(cybTimelock.address, cyb.address, user1.address, 1))
      .to.be.revertedWith("TokenManager: action not signalled")

    await tokenManager.connect(wallet).signalSetGov(cybTimelock.address, cyb.address, user1.address)

    await expect(tokenManager.connect(wallet).setGov(user2.address, cyb.address, user1.address, 1))
      .to.be.revertedWith("TokenManager: action not signalled")

    await expect(tokenManager.connect(wallet).setGov(cybTimelock.address, user0.address, user1.address, 1))
      .to.be.revertedWith("TokenManager: action not signalled")

    await expect(tokenManager.connect(wallet).setGov(cybTimelock.address, cyb.address, user2.address, 1))
      .to.be.revertedWith("TokenManager: action not signalled")

    await expect(tokenManager.connect(wallet).setGov(cybTimelock.address, cyb.address, user1.address, 1 + 1))
      .to.be.revertedWith("TokenManager: action not signalled")

    await expect(tokenManager.connect(wallet).setGov(cybTimelock.address, cyb.address, user1.address, 1))
      .to.be.revertedWith("TokenManager: action not authorized")

    await tokenManager.connect(signer0).signSetGov(cybTimelock.address, cyb.address, user1.address, 1)

    await expect(tokenManager.connect(wallet).setGov(cybTimelock.address, cyb.address, user1.address, 1))
      .to.be.revertedWith("TokenManager: insufficient authorization")

    await expect(cybTimelock.connect(wallet).signalSetGov(cyb.address, user1.address))
      .to.be.revertedWith("CybTimelock: forbidden")

    await tokenManager.connect(signer2).signSetGov(cybTimelock.address, cyb.address, user1.address, 1)

    await expect(cybTimelock.connect(wallet).setGov(cyb.address, user1.address))
      .to.be.revertedWith("CybTimelock: action not signalled")

    await tokenManager.connect(wallet).setGov(cybTimelock.address, cyb.address, user1.address, 1)
    ...
    expect(await cyb.gov()).eq(cybTimelock.address)
    await cybTimelock.connect(wallet).setGov(cyb.address, user1.address)
    expect(await cyb.gov()).eq(user1.address)
  })
})
