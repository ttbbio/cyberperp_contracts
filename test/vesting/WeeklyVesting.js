...
function expectPartisipantEqual(res, cybPurchased, cybClaimed, lastClaimed){
    expect(BigNumber.from(res[0]).eq(cybPurchased), "cybPurchased not equal").to.be.true
    expect(BigNumber.from(res[1]).eq(cybClaimed), "cybClaimed not equal").to.be.true
    expect(BigNumber.from(res[2]).eq(lastClaimed), "lastClaimed not equal").to.be.true
}

describe("WeeklyVesting", () => {
    let weeklyVesting, cyb, usdc, owner, user1, user2;
    ...
    it("should set initial contract variables correctly", async () => {
        expect(await weeklyVesting.cyb()).to.equal(cyb.address);
        ...
        expect(await weeklyVesting.maxCybVesting()).to.equal(MAX_CYB_VESTING);
    });
    ...
    it("owner should withdraw usdc", async () => {
        await weeklyVesting.withdrawTokens(cyb.address, parseEther("100"))
    ...
    it("not owner shouldn`t withdraw usdc", async () => {
        await expect(weeklyVesting.connect(user1).withdrawTokens(cyb.address, parseEther("100"))).to.be.revertedWith("Ownable: caller is not the owner")
    });
