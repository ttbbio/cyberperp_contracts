...
function showPartisipantInfo(res){
    console.log("cybPurchased ", BigNumber.from(res[0]).toString()); 
    console.log("cybClaimed ", BigNumber.from(res[1]).toString()); 
    console.log("lastClaimed ", BigNumber.from(res[2]).toString()); 
}

describe("migrateVesterV1ToV2", () => {
    let vesterV0,vesterV1, vesterV2, cyb, usdc, owner, user1, user2;
    ...
    beforeEach(async () => {
        ...
        vesterV1 = await MigrateVesting.deploy(
            VESTING_WEEKS, // vestingWeeks
            TOKEN_PRICE,
            MAX_CYB_VESTING, // maxCybVesting
            latestTimeStamp + 60 * 60 * 24, // vestingStart (1 day from now)
        ...
        await cyb.transfer(vesterV0.address, MAX_CYB_VESTING);
        await cyb.transfer(vesterV1.address, MAX_CYB_VESTING);
        await cyb.transfer(vesterV2.address, MAX_CYB_VESTING);
    ...
    it("should set initial contract variables correctly", async () => {
        expect(await vesterV2.cyb()).to.equal(cyb.address);
        ...
        expect(await vesterV2.maxCybVesting()).to.equal(MAX_CYB_VESTING);
    });
    ...
    it("owner should be able to withdraw usdc", async () => {
        await vesterV2.withdrawTokens(cyb.address, parseEther("100"))
    ...
    it("not owner shouldn`t be able to withdraw usdc", async () => {
        await expect(vesterV2.connect(user1).withdrawTokens(cyb.address, parseEther("100"))).to.be.revertedWith("Ownable: caller is not the owner")
    });
    ...
    it("bug reproduce on weeklyVesting V1", async () => {
        ...
        await user1WeeklyVesting.buyTokens(parseEther("40000")); // User1 buys 40000 CYB tokens
        ...
        .to.emit(vesterV0, "CybClaimed")
        .withArgs(user1.address, claimPerWeek); // User1 claims 512 CYB tokens (1/78 of the purchased amount)
        const balanceAfter = await user1Cyb.balanceOf(user1.address);
        expect(balanceAfter.sub(balanceBefore).eq(claimPerWeek), "cyb balance hasn`t been changed correctly").to.be.true;
        ...
        const expectedAvailableCyb = claimPerWeek.eq(availableTokens);
    ...
    it("repair bug by weeklyVesting V2", async () => {
        ...
        const user1VesterV1 = vesterV1.connect(user1);
        const user1VesterV2 = vesterV2.connect(user1);
        const buyAmount = parseEther("40000");
        await user1VesterV0.buyTokens(parseEther("40000")); // User1 buys 40000 CYB tokens
        // Fast forward to vesting start
        ...
        const claimPerWeek = buyAmount.div(VESTING_WEEKS);  // = 512 820512820512820512
        const balanceBefore = await user1Cyb.balanceOf(user1.address);
        await expect(user1VesterV1.claimTokens())
        .to.emit(vesterV1, "CybClaimed")
        .withArgs(user1.address, claimPerWeek); // User1 claims 512 CYB tokens (1/78 of the purchased amount)
        const balanceAfter = await user1Cyb.balanceOf(user1.address);
        expect(balanceAfter.sub(balanceBefore).eq(claimPerWeek), "cyb balance hasn`t been changed correctly").to.be.true;

        let res = await vesterV1.participants(user1.address);
        latestTimeStamp = await helpers.time.latest();
        expectPartisipantEqual(res, buyAmount, claimPerWeek, latestTimeStamp);
        ...
        const remainingCyb = BigNumber.from("512820512820512820576");
        expect(vesterV2AvailableTokens, "tokens are not available after second vesting week passed").to.equal(remainingCyb) 
    ...
    it("should claim tokens referencing to the state of migrated contract", async () => {
        ...
        await user1MigrateVesting.buyTokens(parseEther("780")); // User1 buys 40 CYB tokens
        res = await vesterV2.participants(user1.address);
        console.log(`res 1: ${res[0]} ${res[1]} ${res[2]}`)
    ...
    it("should allow user to claim tokens after vesting starts", async () => {
        ...
        const user1WeeklyVesting = vesterV2.connect(user1);
        await user1WeeklyVesting.buyTokens(parseEther("780")); // User1 buys 40 CYB tokens
        let res = await vesterV2.participants(user1.address);
        console.log(`res 1: ${res[0]} ${res[1]} ${res[2]}`)
