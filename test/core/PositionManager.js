...
const { deployContract, deployVault, deployVaultPriceFeed, deployCyberLPManager, deployTimelock, deployTimeDistributor } = require("../shared/fixtures")
  ...
  beforeEach(async () => {
    ...
    deployTimelockCallback = async () => {
      return await deployTimelock([
        ...
        ethers.constants.AddressZero, // _cyberLPManager
...
describe("PositionManager next short data calculations", function () {
  ...
  async function debugState(label = "") {
    ...
    const aum = await cyberLPManager.getAum(true)
  ...
  it("PositionManager and CyberLPManager init with shortsTracker", async () => {
    const [
      positionManagerShortTracker,
      cyberLPManagerShortTracker,
      avgeragePrice,
      size
    ] = await Promise.all([
      positionManager.shortsTracker(),
      cyberLPManager.shortsTracker(),
    ...
    expect(cyberLPManagerShortTracker, 'cyberLPManager shortsTracker').eq(shortsTracker.address)
  ...
  it("updates global short average prices on position decreases", async () => {
    await shortsTracker.setIsGlobalShortDataReady(true)
    await cyberLPManager.setShortsTrackerAveragePriceWeight(10000)
    expect(await cyberLPManager.shortsTrackerAveragePriceWeight()).to.be.equal(10000)
    ...
    let aumBefore = await cyberLPManager.getAum(true)
    ...
    let aumAfter = await cyberLPManager.getAum(true)
    ...
    aumBefore = await cyberLPManager.getAum(true)
    ...
    aumAfter = await cyberLPManager.getAum(true)
  ...
  it("updates global short average prices on soft liquidation", async () => {
    ...
    await positionManager.setLiquidator(user1.address, true)
    const aumBefore = await cyberLPManager.getAum(true)
    ...
    const aumAfter = await cyberLPManager.getAum(true)
