...
describe("Vault.decreaseLongPosition", function () {
  ...
  it("decreasePosition long", async () => {
    ...
    expect(await cyberLPManager.getAumInUsdg(false)).eq("103180923000000000000") // 103.917746
    expect(await cyberLPManager.getAumInUsdg(true)).eq("103180923000000000000") // 107.058666
  ...
  it("decreasePosition long aum", async () => {
    ...
    expect(await cyberLPManager.getAumInUsdg(false)).eq("7227000000000000000000") // 7227
    expect(await cyberLPManager.getAumInUsdg(true)).eq("7227000000000000000000") // 7227

    await vault.connect(user0).decreasePosition(user0.address, eth.address, eth.address, toUsd(0), toUsd(500), true, user2.address)

    expect(await cyberLPManager.getAumInUsdg(false)).eq("7227000000000000000250") // 7227.00000000000000025
    expect(await cyberLPManager.getAumInUsdg(true)).eq("7227000000000000000250") // 7227.00000000000000025
