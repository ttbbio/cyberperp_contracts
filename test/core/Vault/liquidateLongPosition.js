...
describe("Vault.liquidateLongPosition", function () {
  ...
  it("liquidate long", async () => {
    ...
    expect(await cyberLPManager.getAumInUsdg(false)).eq("96121175000000000000") // 99.064997
    expect(await cyberLPManager.getAumInUsdg(true)).eq("96121175000000000000") // 101.418485
    ...
    expect(await cyberLPManager.getAumInUsdg(false)).eq("84516420000000000000") // 101.522097
    expect(await cyberLPManager.getAumInUsdg(true)).eq("84516420000000000000") // 114.113985
