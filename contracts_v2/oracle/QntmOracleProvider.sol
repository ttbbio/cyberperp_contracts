...
contract QntmOracleProvider is RoleModule, IOracleProvider {
    ...
    function getOraclePrice(
        ...
        QntmOracleUtils.Report memory report = abi.decode(data, (QntmOracleUtils.Report));
        address[] memory signers = _getSigners(report.signerInfo);

        if (report.minOracleBlockNumber > report.maxOracleBlockNumber) {
            revert Errors.QntmInvalidMinMaxBlockNumber(report.minOracleBlockNumber, report.maxOracleBlockNumber);
        }

        if (report.maxOracleBlockNumber >= Chain.currentBlockNumber()) {
            revert Errors.QntmInvalidBlockNumber(report.maxOracleBlockNumber, Chain.currentBlockNumber());
        }
        ...
        for (uint256 i = 0; i < signers.length; i++) {
            ...
            if (report.minPrices[i - 1] > report.minPrices[i]) {
                revert Errors.QntmMinPricesNotSorted(token, report.minPrices[i], report.minPrices[i - 1]);
            }

            // validate that maxPrices are sorted in ascending order
            if (report.maxPrices[i - 1] > report.maxPrices[i]) {
                revert Errors.QntmMaxPricesNotSorted(token, report.maxPrices[i], report.maxPrices[i - 1]);
            }
        }
        ...
        for (uint256 i = 0; i < signers.length; i++) {
            ...
            if (minPrice > maxPrice) {
                revert Errors.InvalidQntmSignerMinMaxPrice(minPrice, maxPrice);
            }

            QntmOracleUtils.validateSigner(
                salt,
                report,
                token,
                minPrice,
        ...
        if (medianMinPrice == 0 || medianMaxPrice == 0) {
            revert Errors.InvalidQntmOraclePrice(token);
        }

        if (medianMinPrice > medianMaxPrice) {
            revert Errors.InvalidQntmMedianMinMaxPrice(medianMinPrice, medianMaxPrice);
        }
    ...
    function _getSigners(
        ...
        if (signers.length < dataStore.getUint(Keys.MIN_ORACLE_SIGNERS)) {
            revert Errors.QntmMinOracleSigners(signers.length, dataStore.getUint(Keys.MIN_ORACLE_SIGNERS));
        }

        if (signers.length > MAX_SIGNERS) {
            revert Errors.QntmMaxOracleSigners(signers.length, MAX_SIGNERS);
        }
        ...
        for (uint256 i; i < signers.length; i++) {
            ...
            if (signerIndex >= MAX_SIGNER_INDEX) {
                revert Errors.QntmMaxSignerIndex(signerIndex, MAX_SIGNER_INDEX);
            }
            ...
            if (signers[i] == address(0)) {
                revert Errors.QntmEmptySigner(signerIndex);
            }
        }
