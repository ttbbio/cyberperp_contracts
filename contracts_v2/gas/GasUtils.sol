...
library GasUtils {
    ...
    // @dev the estimated gas limit for qntlv deposits
    // @param dataStore DataStore
    // @param deposit the deposit to estimate the gas limit for
    function estimateExecuteQntlvDepositGasLimit(DataStore dataStore, QntlvDeposit.Props memory qntlvDeposit, uint256 marketCount) internal view returns (uint256) {
        // qntlv deposit execution gas consumption depends on the amount of markets
        uint256 gasPerQntlvPerMarket = dataStore.getUint(Keys.qntlvPerMarketGasLimitKey());
        uint256 gasForQntlvMarkets = gasPerQntlvPerMarket * marketCount;
        uint256 qntlvDepositGasLimit = dataStore.getUint(Keys.qntlvDepositGasLimitKey());

        uint256 gasLimit = qntlvDepositGasLimit + qntlvDeposit.callbackGasLimit() + gasForQntlvMarkets;

        if (qntlvDeposit.isMarketTokenDeposit()) {
            // user provided QNTM, no separate deposit will be created and executed in this case
            return gasLimit;
        }

        uint256 gasPerSwap = dataStore.getUint(Keys.singleSwapGasLimitKey());
        uint256 swapCount = qntlvDeposit.longTokenSwapPath().length + qntlvDeposit.shortTokenSwapPath().length;
        uint256 gasForSwaps = swapCount * gasPerSwap;

        return gasLimit + dataStore.getUint(Keys.depositGasLimitKey()) + gasForSwaps;
    }

    // @dev the estimated gas limit for qntlv withdrawals
    // @param dataStore DataStore
    // @param withdrawal the withdrawal to estimate the gas limit for
    function estimateExecuteQntlvWithdrawalGasLimit(DataStore dataStore, QntlvWithdrawal.Props memory qntlvWithdrawal, uint256 marketCount) internal view returns (uint256) {
        // qntlv withdrawal execution gas consumption depends on the amount of markets
        uint256 gasPerQntlvPerMarket = dataStore.getUint(Keys.qntlvPerMarketGasLimitKey());
        uint256 gasForQntlvMarkets = gasPerQntlvPerMarket * marketCount;
        uint256 qntlvWithdrawalGasLimit = dataStore.getUint(Keys.qntlvWithdrawalGasLimitKey());

        uint256 gasLimit = qntlvWithdrawalGasLimit + qntlvWithdrawal.callbackGasLimit() + gasForQntlvMarkets;

        uint256 gasPerSwap = dataStore.getUint(Keys.singleSwapGasLimitKey());
        uint256 swapCount = qntlvWithdrawal.longTokenSwapPath().length + qntlvWithdrawal.shortTokenSwapPath().length;
        uint256 gasForSwaps = swapCount * gasPerSwap;
    ...
    function estimateExecuteQntlvShiftGasLimit(DataStore dataStore) internal view returns (uint256) {
        return dataStore.getUint(Keys.qntlvShiftGasLimitKey());
    }
