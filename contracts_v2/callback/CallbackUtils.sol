...
import "./IQntlvDepositCallbackReceiver.sol";
import "./IQntlvWithdrawalCallbackReceiver.sol";
...
library CallbackUtils {
    ...
    event AfterQntlvDepositExecutionError(bytes32 key, QntlvDeposit.Props qntlvDeposit);
    event AfterQntlvDepositCancellationError(bytes32 key, QntlvDeposit.Props qntlvDeposit);
    event AfterQntlvWithdrawalExecutionError(bytes32 key, QntlvWithdrawal.Props qntlvWithdrawal);
    event AfterQntlvWithdrawalCancellationError(bytes32 key, QntlvWithdrawal.Props qntlvWithdrawal);
    ...
    function afterQntlvDepositExecution(
        ...
        if (!isValidCallbackContract(qntlvDeposit.callbackContract())) {
            return;
        }

        validateGasLeftForCallback(qntlvDeposit.callbackGasLimit());

        try IQntlvDepositCallbackReceiver(qntlvDeposit.callbackContract()).afterQntlvDepositExecution{ gas: qntlvDeposit.callbackGasLimit() }(
            key,
            qntlvDeposit,
            eventData
        ) {
        } catch {
            emit AfterQntlvDepositExecutionError(key, qntlvDeposit);
        }
    }
    ...
    function afterQntlvDepositCancellation(
        ...
        if (!isValidCallbackContract(qntlvDeposit.callbackContract())) { return; }

        validateGasLeftForCallback(qntlvDeposit.callbackGasLimit());

        try IQntlvDepositCallbackReceiver(qntlvDeposit.callbackContract()).afterQntlvDepositCancellation{ gas: qntlvDeposit.callbackGasLimit() }(
            key,
            qntlvDeposit,
            eventData
        ) {
        } catch {
            emit AfterQntlvDepositCancellationError(key, qntlvDeposit);
        }
    }
    ...
    function afterQntlvWithdrawalExecution(
        ...
        if (!isValidCallbackContract(qntlvWithdrawal.callbackContract())) { return; }

        validateGasLeftForCallback(qntlvWithdrawal.callbackGasLimit());

        try IQntlvWithdrawalCallbackReceiver(qntlvWithdrawal.callbackContract()).afterQntlvWithdrawalExecution{ gas: qntlvWithdrawal.callbackGasLimit() }(
            key,
            qntlvWithdrawal,
            eventData
        ) {
        } catch {
            emit AfterQntlvWithdrawalExecutionError(key, qntlvWithdrawal);
        }
    }
    ...
    function afterQntlvWithdrawalCancellation(
        ...
        try IQntlvWithdrawalCallbackReceiver(qntlvWithdrawal.callbackContract()).afterQntlvWithdrawalCancellation{ gas: qntlvWithdrawal.callbackGasLimit() }(
            key,
            qntlvWithdrawal,
            eventData
        ) {
        } catch {
            emit AfterQntlvWithdrawalCancellationError(key, qntlvWithdrawal);
        }
    }
