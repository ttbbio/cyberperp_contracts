...
library Errors {
    ...
    // QntlvDepositStoreUtils errors
    error QntlvDepositNotFound(bytes32 key);
    // QntlvShiftStoreUtils errors
    error QntlvShiftNotFound(bytes32 key);
    // QntlvWithdrawalStoreUtils errors
    error QntlvWithdrawalNotFound(bytes32 key);
    // QntlvDepositUtils errors
    error EmptyQntlvDepositAmounts();
    error EmptyQntlvMarketAmount();
    error EmptyQntlvDeposit();
    error InvalidMinQntlvTokensForFirstQntlvDeposit(uint256 minQntlvTokens, uint256 expectedMinQntlvTokens);
    error InvalidReceiverForFirstQntlvDeposit(address receiver, address expectedReceiver);
    // QntlvWithdrawalUtils errors
    error EmptyQntlvWithdrawal();
    error EmptyQntlvWithdrawalAmount();
    // QntlvUtils errors
    error EmptyQntlv(address qntlv);
    error EmptyQntlvTokenSupply();
    error QntlvNegativeMarketPoolValue(address qntlv, address market);
    error QntlvUnsupportedMarket(address qntlv, address market);
    error QntlvDisabledMarket(address qntlv, address market);
    error QntlvEnabledMarket(address qntlv, address market);
    error QntlvNonZeroMarketBalance(address qntlv, address market);
    error QntlvMaxMarketCountExceeded(address qntlv, uint256 qntlvMaxMarketCount);
    error QntlvMaxMarketTokenBalanceUsdExceeded(address qntlv, address market, uint256 maxMarketTokenBalanceUsd, uint256 marketTokenBalanceUsd);
    error QntlvMaxMarketTokenBalanceAmountExceeded(address qntlv, address market, uint256 maxMarketTokenBalanceAmount, uint256 marketTokenBalanceAmount);
    error QntlvInsufficientMarketTokenBalance(address qntlv, address market, uint256 marketTokenBalance, uint256 marketTokenAmount);
    error QntlvMarketAlreadyExists(address qntlv, address market);
    error QntlvInvalidLongToken(address qntlv, address provided, address expected);
    error QntlvInvalidShortToken(address qntlv, address provided, address expected);
    // QntlvShiftUtils
    error QntlvShiftMaxPriceImpactExceeded(uint256 effectivePriceImpactFactor, uint256 qntlvMaxShiftPriceImpactFactor);
    error QntlvShiftIntervalNotYetPassed(uint256 currentTimestamp, uint256 lastQntlvShiftExecutedAt, uint256 qntlvShiftMinInterval);
    // QntlvFactory
    error QntlvAlreadyExists(bytes32 salt, address qntlv);
    error QntlvSymbolTooLong();
    error QntlvNameTooLong();
    // QntlvStoreUtils
    error QntlvNotFound(address key);
    ...
    // QntlpMigrator errors
    error InvalidQntlpAmount(uint256 totalQntlpAmountToRedeem, uint256 totalQntlpAmount);
    error InvalidExecutionFeeForMigration(uint256 totalExecutionFee, uint256 msgValue);

    // QntlvHandler errors
    error InvalidQntlvDepositInitialLongToken(address initialLongToken);
    error InvalidQntlvDepositInitialShortToken(address initialShortToken);
    error InvalidQntlvDepositSwapPath(uint256 longTokenSwapPathLength, uint256 shortTokenSwapPathLength);
    error MinQntlvTokens(uint256 received, uint256 expected);
    ...
    error QntmEmptySigner(uint256 signerIndex);
    ...
    error QntmInvalidBlockNumber(uint256 minOracleBlockNumber, uint256 currentBlockNumber);
    error QntmInvalidMinMaxBlockNumber(uint256 minOracleBlockNumber, uint256 maxOracleBlockNumber);
    ...
    error QntmMinOracleSigners(uint256 oracleSigners, uint256 minOracleSigners);
    error QntmMaxOracleSigners(uint256 oracleSigners, uint256 maxOracleSigners);
    error BlockNumbersNotSorted(uint256 minOracleBlockNumber, uint256 prevMinOracleBlockNumber);
    error QntmMinPricesNotSorted(address token, uint256 price, uint256 prevPrice);
    error QntmMaxPricesNotSorted(address token, uint256 price, uint256 prevPrice);
    ...
    error QntmMaxSignerIndex(uint256 signerIndex, uint256 maxSignerIndex);
    error InvalidQntmOraclePrice(address token);
    error InvalidQntmSignerMinMaxPrice(uint256 minPrice, uint256 maxPrice);
    error InvalidQntmMedianMinMaxPrice(uint256 minPrice, uint256 maxPrice);
    ...
    // OracleUtils errors
    error InvalidQntmSignature(address recoveredSigner, address expectedSigner);
