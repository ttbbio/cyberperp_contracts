...
contract WeeklyVesting is Ownable_0_8_0 {
    ...
    uint256 public totalCybPurchased;
    ...
    function getAvailableTokens(address user) public view returns (uint256) {
        ...
        uint256 tokensPerWeek = participant.cybPurchased / vestingWeeks;
        uint256 tokensToClaim = tokensPerWeek * weeksPassed;

        return
            (participant.cybClaimed + tokensToClaim > participant.cybPurchased)
                ? participant.cybPurchased - participant.cybClaimed
                : tokensToClaim;
    }
