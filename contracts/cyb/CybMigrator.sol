...
import "./interfaces/ICybIou.sol";
import "./interfaces/IAmmRouter.sol";
import "./interfaces/ICybMigrator.sol";

contract CybMigrator is ReentrancyGuard, ICybMigrator {
    ...
    modifier onlyAdmin() {
        require(msg.sender == admin, "CybMigrator: forbidden");
        _;
    }

    modifier onlySigner() {
        require(isSigner[msg.sender], "CybMigrator: forbidden");
        _;
    }
    ...
    function initialize(
        ...
        require(!isInitialized, "CybMigrator: already initialized");
        require(_whitelistedTokens.length == _iouTokens.length, "CybMigrator: invalid _iouTokens.length");
        require(_whitelistedTokens.length == _prices.length, "CybMigrator: invalid _prices.length");
        require(_whitelistedTokens.length == _caps.length, "CybMigrator: invalid _caps.length");
        require(_lpTokens.length == _lpTokenAs.length, "CybMigrator: invalid _lpTokenAs.length");
        require(_lpTokens.length == _lpTokenBs.length, "CybMigrator: invalid _lpTokenBs.length");
    ...
    function migrate(
        ...
        require(isMigrationActive, "CybMigrator: migration is no longer active");
        require(whitelistedTokens[_token], "CybMigrator: token not whitelisted");
        require(_tokenAmount > 0, "CybMigrator: invalid tokenAmount");

        if (hasMaxMigrationLimit) {
            migratedAmounts[msg.sender][_token] = migratedAmounts[msg.sender][_token].add(_tokenAmount);
            require(migratedAmounts[msg.sender][_token] <= maxMigrationAmounts[msg.sender][_token], "CybMigrator: maxMigrationAmount exceeded");
        }

        uint256 tokenPrice = getTokenPrice(_token);
        uint256 mintAmount = _tokenAmount.mul(tokenPrice).div(cybPrice);
        require(mintAmount > 0, "CybMigrator: invalid mintAmount");

        tokenAmounts[_token] = tokenAmounts[_token].add(_tokenAmount);
        require(tokenAmounts[_token] < caps[_token], "CybMigrator: token cap exceeded");
        ...
        if (lpTokens[_token]) {
            ...
            require(tokenA != address(0), "CybMigrator: invalid tokenA");
            require(tokenB != address(0), "CybMigrator: invalid tokenB");
        ...
        ICybIou(iouToken).mint(msg.sender, mintAmount);
    }
    ...
    function signApprove(address _token, address _spender, uint256 _amount, uint256 _nonce) external nonReentrant onlySigner {
        ...
        require(!signedActions[msg.sender][action], "CybMigrator: already signed");
    ...
    function getTokenPrice(address _token) public view returns (uint256) {
        ...
        require(price != 0, "CybMigrator: invalid token price");
        return price;
    }

    function getIouToken(address _token) public view returns (address) {
        address iouToken = iouTokens[_token];
        require(iouToken != address(0), "CybMigrator: invalid iou token");
        return iouToken;
    }
    ...
    function _validateAction(bytes32 _action) private view {
        require(pendingActions[_action], "CybMigrator: action not signalled");
    }
    ...
    function _validateAuthorization(bytes32 _action) private view {
        ...
        if (count == 0) {
            revert("CybMigrator: action not authorized");
        }
        require(count >= minAuthorizations, "CybMigrator: insufficient authorization");
    }

    function _clearAction(bytes32 _action, uint256 _nonce) private {
        require(pendingActions[_action], "CybMigrator: invalid _action");
