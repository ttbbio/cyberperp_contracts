...
import "../core/interfaces/ICyberLPManager.sol";
...
import "../staking/interfaces/IRewardRouterV4_0_6_12.sol";
...
contract Timelock is ITimelock {
    ...
    uint256 public constant MAX_LEVERAGE_VALIDATION = 1000000;
    ...
    function initCyberLPManager() external onlyAdmin {
        ICyberLPManager _cyberLPManager = ICyberLPManager(cyberLPManager);

        IMintable cyberLP = IMintable(_cyberLPManager.cyberLP());
        cyberLP.setMinter(cyberLPManager, true);

        IUSDG usdg = IUSDG(_cyberLPManager.usdg());
        usdg.addVault(cyberLPManager);

        IVault vault = _cyberLPManager.vault();
        vault.setManager(cyberLPManager, true);
    }

    function initRewardRouter() external onlyAdmin {
        IRewardRouterV4_0_6_12 _rewardRouter = IRewardRouterV4_0_6_12(rewardRouter);

        IHandlerTarget(_rewardRouter.feeCyberLPTracker()).setHandler(rewardRouter, true);
        IHandlerTarget(_rewardRouter.stakedCyberLPTracker()).setHandler(rewardRouter, true);
        IHandlerTarget(cyberLPManager).setHandler(rewardRouter, true);
    }
    ...
    function updateUsdgSupply(uint256 usdgAmount) external onlyKeeperAndAbove {
        address usdg = ICyberLPManager(cyberLPManager).usdg();
        uint256 balance = IERC20(usdg).balanceOf(cyberLPManager);

        IUSDG(usdg).addVault(address(this));

        if (usdgAmount > balance) {
            uint256 mintAmount = usdgAmount.sub(balance);
            IUSDG(usdg).mint(cyberLPManager, mintAmount);
        } else {
            uint256 burnAmount = balance.sub(usdgAmount);
            IUSDG(usdg).burn(cyberLPManager, burnAmount);
        }
    ...
    function setShortsTrackerAveragePriceWeight(uint256 _shortsTrackerAveragePriceWeight) external onlyAdmin {
        ICyberLPManager(cyberLPManager).setShortsTrackerAveragePriceWeight(_shortsTrackerAveragePriceWeight);
    }

    function setCyberLPCooldownDuration(uint256 _cooldownDuration) external onlyAdmin {
        require(_cooldownDuration < 2 hours, "Timelock: invalid _cooldownDuration");
        ICyberLPManager(cyberLPManager).setCooldownDuration(_cooldownDuration);
    }
    ...
    function _validateAction(bytes32 _action) private view {
        ...
        require(pendingActions[_action] < blockInfo.getBlockTimestamp(), "Timelock: action time not yet passed");
    }
